...
RSpec.describe Authentication, type: :model do
  ...
  it 'fails if a password was never set' do
    ...
    expect(auth.errors.added?(:base, :never_set_password)).to eq(true)
  end
  ...
  it 'fails if no valid email or mobile number is provided' do
    ...
    expect(auth.run).to eq(false)
    expect(auth.errors.added?(:base, :email_or_mobile_invalid)).to eq(true)
  end
  ...
  it 'fails if the email does not exist' do
    ...
    expect(auth.run).to eq(false)
    expect(auth.errors.added?(:base, :email_not_found)).to eq(true)
  end
  ...
  it 'fails if the mobile number does not exist' do
    ...
    expect(auth.run).to eq(false)
    expect(auth.errors.added?(:base, :phone_not_found)).to eq(true)
  end
  ...
  it 'fails if the password is wrong' do
    ...
    expect(auth.run).to eq(false)
    expect(auth.errors.added?(:base, :invalid_password)).to eq(true)
  end
end
