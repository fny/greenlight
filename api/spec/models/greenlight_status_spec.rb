...
RSpec.describe GreenlightStatus, type: :model do
  ...
  describe '#submitted_at=' do
    it 'assigns the same subission date and follow up date when before the cutoff' do
      ...
      expect(status.submission_date).to eq(Date.today)
      expect(status.follow_up_date).to eq(Date.today + 1.day)
    end
    ...
    it 'assigns the same subission date and follow up date when before the cutoff' do
      ...
      expect(status.submission_date).to eq(Date.today)
      expect(status.follow_up_date).to eq(Date.today + 2.day)
    end
  end
end
