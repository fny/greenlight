...
export class Fixture {
  ...
  constructor(type: string, data: any) {
    ...
    if (type === 'user' || type === 'users') {
      this._data.authToken = crypto.createHash('md5').update(this._data.id).digest("hex")
    }
    this.type = type
    this.id = data.id
  }
...
export function where(model: string, query: any) {
  const records = all(model)
  return records.filter(r => {
    const attributes = r.attributes()
    let all = true
    for (let [key, value] of query) {
      all = all && attributes[key] === value
    }
    return all
  })
}
...
export function findWhere(model: string, query: any): Fixture {
  const records = where(model, query)
  if (records.length === 0) {
    throw new Error(`Fixture not found for model ${model} with query ${JSON.stringify(query)}`)
  }
  return records[0]
}
