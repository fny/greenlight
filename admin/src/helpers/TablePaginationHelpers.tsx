...
export function getHandlerTableChange(setQueryParams: any) {
  return (type: any, { page, sizePerPage, sortField, sortOrder, data }: any) => {
...
export function PleaseWaitMessage({ entities }: { entities: any[] }) {
  return <>{entities === null && <div>Please wait...</div>}</>;
}

export function NoRecordsFoundMessage({ entities }: { entities: any[] }) {
