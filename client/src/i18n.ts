...
export class MyI18n {
  ...
  _(message: string | MessageDescriptor) {
    ...
    if (translated === undefined) {
      console.error(`Missing translation: ${JSON.stringify(message)}`)
    }

    if (translated === null) {
      console.error(`Translation unset: ${JSON.stringify(message)}`)
    }

    if (translated === '') {
      console.error(`Translation is empty string: ${JSON.stringify(message)}`)
    }
